...
from src.application.common.exceptions import RefreshWalletStatsTaskNotFoundException
...
class GetRefreshWalletStatsStatusHandler:
    ...
    async def __call__(self, task_id) -> GetRefreshWalletStatsStatusResponse:
        result = AsyncResult(task_id, app=celery_app)
        task_status = TaskStatus(str(result.status).lower())
        return GetRefreshWalletStatsStatusResponse(
            task_id=task_id, task_status=task_status, task_result=str(result.result)
        )
