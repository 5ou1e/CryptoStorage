...
def generate_wallet_stats_filter_fields(prefix: str):
    return {
        ...
        f"{prefix}__total_profit_usd__gte": (
            Optional[int],
            Field(Query(None, description=f"")),
        ),
        f"{prefix}__total_profit_usd__lte": (
            Optional[int],
            Field(Query(None, description=f"")),
        ),
        f"{prefix}__total_profit_multiplier__gte": (
            Optional[int],
            Field(Query(None, description=f"")),
        ),
        f"{prefix}__total_profit_multiplier__lte": (
            Optional[int],
            Field(Query(None, description=f"")),
        ),
        ...
        f"{prefix}__pnl_gt_5x_percent__gte": (
            Optional[float],
            Field(Query(None, description=f"")),
        ),
        f"{prefix}__pnl_gt_5x_percent__lte": (
            Optional[float],
            Field(Query(None, description=f"")),
        ),
        f"{prefix}__token_first_buy_median_price_usd__gte": (
            Optional[float],
            Field(Query(None, description=f"")),
        ),
        f"{prefix}__token_first_buy_median_price_usd__lte": (
            Optional[float],
            Field(Query(None, description=f"")),
        ),
        f"{prefix}__token_buy_sell_duration_median__gte": (
            Optional[float],
            Field(Query(None, description=f"")),
        ),
        f"{prefix}__token_buy_sell_duration_median__lte": (
            Optional[float],
            Field(Query(None, description=f"")),
        ),
    }
