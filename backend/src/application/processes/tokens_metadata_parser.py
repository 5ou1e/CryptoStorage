...
async def set_token_metadata(token, metadata, json_metadata):
    ...
    token.name = metadata.get("name", "").replace("\x00", "")
    token.symbol = metadata.get("symbol", "").replace("\x00", "")
    token.uri = metadata.get("uri", "").replace("\x00", "")

    if isinstance(json_metadata, dict):
        logo = str(json_metadata.get("image", "")).replace("\u0000", "").replace("\x00", "")
        created_on = str(json_metadata.get("createdOn", "")).replace("\u0000", "").replace("\x00", "")
