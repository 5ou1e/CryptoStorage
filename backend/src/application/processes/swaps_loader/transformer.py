...
def builds_objects(
    ...
    for swap in swaps:
        try:
            ...
            quote_amount = swap["swap_from_amount"] if is_buy_swap else swap["swap_to_amount"]
            quote_amount = Decimal(str(quote_amount)) if quote_amount is not None else Decimal("0")

            token_amount = swap["swap_to_amount"] if is_buy_swap else swap["swap_from_amount"]
            token_amount = Decimal(str(token_amount)) if token_amount is not None else Decimal("0")
            ...
            tx_id = swap["tx_id"]
            logger.exception(f"Ошибка при извлечении данных свапа '{tx_id}' : {e}", exc_info=True)
            logger.error(str(swap))
            continue
