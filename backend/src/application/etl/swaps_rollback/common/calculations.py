...
def calculate_wallet_token(wt: WalletToken, activities: list[Swap]) -> None:
    ...
    for activity in activities:
        if activity.event_type == SwapEventType.BUY:
            ...
            wt.total_buy_amount_token -= (
                activity.token_amount if activity.token_amount else 0
            )
            if not wt.first_buy_timestamp or (
                activity.timestamp < wt.first_buy_timestamp
            ):
                wt.first_buy_timestamp = activity.timestamp
                if activity.token_amount:
                    wt.first_buy_price_usd = (
                        activity.price_usd
            ...
            wt.total_sell_amount_token -= (
                activity.token_amount if activity.token_amount else 0
            )
            if not wt.first_sell_timestamp or (
                activity.timestamp < wt.first_sell_timestamp
            ):
                wt.first_sell_timestamp = activity.timestamp
                if activity.token_amount:
                    wt.first_sell_price_usd = (
                        activity.price_usd
                        * activity.quote_amount
                        / activity.token_amount
                    )
            ...
            continue
        if not wt.last_activity_timestamp or (
            activity.timestamp > wt.last_activity_timestamp
        ):
    ...
    if (
        ...
        and wt.first_sell_timestamp
        and (wt.first_buy_timestamp <= wt.first_sell_timestamp)
    ):
        wt.first_buy_sell_duration = int(
            (wt.first_sell_timestamp - wt.first_buy_timestamp).total_seconds()
        )
