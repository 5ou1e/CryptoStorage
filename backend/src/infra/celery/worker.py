...
def setup_tasks_on_startup(sender, **kwargs):
    ...
    swaps_loader_bitquery_process_task.apply_async(task_name="Сбор свапов с Bitquery")
    swaps_loader_delays_alerter_task.apply_async(task_name="Процесс отслеживания задержек в сборе свапов")
    collect_sol_prices_task.apply_async(task_name="Сбор цен Solana-токена с Binance")
