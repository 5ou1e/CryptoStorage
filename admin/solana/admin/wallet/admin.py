...
from ...models.wallet import WalletCopyable, WalletCopyableBase
...
from .views import (WalletCopyableView, WalletStatisticBuyPriceGt15kView,
                    WalletStatisticView)
...
class WalletAdmin(
    ...
    def get_list_display(self, request):
        def get_displays_for_active_period(period=None) -> list[str]:
            ...
            return [
                (
                    item.format(stats_rel_name=stats_rel_name)
                    if "{stats_rel_name}" in item
                    else item
                )
                for item in base
            ]
    ...
    def get_urls(self):
        return [
            path(
                "<str:wallet_address>/",
                WalletStatisticView.as_view(model_admin=self),
...
class WalletBuyPriceGt15kAdmin(
    ...
    def get_urls(self):
        return [
            path(
                "<str:wallet_address>/",
                WalletStatisticBuyPriceGt15kView.as_view(model_admin=self),
                name="wallet_statistic_view",
            ),
        ] + super().get_urls()
...
@admin.register(WalletCopyable)
class WalletCopyableAdmin(
    BaseAdminModel,
    WalletBuyPriceGt15kActionsMixin,
    WalletDisplaysMixin,
    WalletStatsDisplaysMixin,
    ...
    def get_urls(self):
        return [
            path(
                "<str:wallet_address>/",
                WalletCopyableView.as_view(model_admin=self),
                name="wallet_statistic_view",
            ),
        ] + super().get_urls()
