...
def get_wallet_statistics_data(user, wallet, use_buy_price_gt_20k_stats=False):
    ...
    data['last_stats_updated_time_ago'] = {
        'display_value': f_d(now() - wallet.last_stats_check) + " назад" if wallet.last_stats_check else '-',
    ...
    for period in periods_data:
        ...
        period_data['winrate'] = {
            'display_value': f_n(period_stats.winrate,
                                           suffix=' %') if period_stats.winrate is not None else '-',
            'value': period_stats.winrate,
        }
        period_data['total_profit'] = {
            'display_value': f_n(period_stats.total_profit_usd, suffix=' $', add_sign=True),
            'value': period_stats.total_profit_usd,
        }
        period_data['total_profit_multiplier'] = {
            'display_value': f_n(period_stats.total_profit_multiplier, suffix=' %', add_sign=True),
        ...
        if period_stats.token_sell_without_buy is not None and period_stats.total_token:
            token_sell_without_buy_percent = period_stats.token_sell_without_buy / period_stats.total_token * 100
        else:
        ...
        if period_stats.token_buy_without_sell is not None and period_stats.total_token:
            token_buy_without_sell_percent = period_stats.token_buy_without_sell / period_stats.total_token * 100
        else:
        ...
        if period_stats.token_with_sell_amount_gt_buy_amount is not None and period_stats.total_token:
            token_with_sell_amount_gt_buy_amount_percent = period_stats.token_with_sell_amount_gt_buy_amount / period_stats.total_token * 100
        else:
        ...
        total_token_transactions = period_stats.total_token_buys + period_stats.total_token_sales if (period_stats.total_token_buys is not None and period_stats.total_token_sales is not None ) else None
        ...
        period_data['token_avg_buy_amount'] = f_n(period_stats.token_avg_buy_amount, suffix=" $")
        ...
        period_data['token_buy_sell_duration_avg'] = {
            'display_value': f_d(
                int(period_stats.token_buy_sell_duration_avg)) if period_stats.token_buy_sell_duration_avg else None,
            'value': timedelta(seconds=int(
                period_stats.token_buy_sell_duration_avg)) if period_stats.token_buy_sell_duration_avg else None
        }
        ...
        period_data['token_first_buy_avg_price_usd'] = {
            'display_value': f_n(period_stats.token_first_buy_avg_price_usd, suffix=" $", decimals=10,
                                           subscript=True),
            'value': period_stats.token_first_buy_avg_price_usd
        }
        period_data['token_first_buy_median_price_usd'] = {
            'display_value': f_n(period_stats.token_first_buy_median_price_usd, suffix=" $", decimals=10,
                                           subscript=True),
            'value': period_stats.token_first_buy_median_price_usd
        }

        period_data['pnl_gt_5x_num'] = f"{f_n(period_stats.pnl_gt_5x_num)} ({f_n(period_stats.pnl_gt_5x_percent, suffix=' %')})"
        period_data['pnl_2x_5x_num'] = f"{f_n(period_stats.pnl_2x_5x_num)} ({f_n(period_stats.pnl_2x_5x_percent, suffix=' %')})"
        period_data['pnl_lt_2x_num'] = f"{f_n(period_stats.pnl_lt_2x_num)} ({f_n(period_stats.pnl_lt_2x_percent, suffix=' %')})"
        period_data['pnl_minus_dot5_0x_num'] = f"{f_n(period_stats.pnl_minus_dot5_0x_num)} ({f_n(period_stats.pnl_minus_dot5_0x_percent, suffix=' %')})"
        period_data['pnl_lt_minus_dot5_num'] = f"{f_n(period_stats.pnl_lt_minus_dot5_num)} ({f_n(period_stats.pnl_lt_minus_dot5_percent, suffix=' %')})"
