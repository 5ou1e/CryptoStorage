...
class WalletTokenStatistic(models.Model):
    ...
    def token_name_display(self):
        last_activity_ago = formatted_duration(int(now().timestamp()) - self.last_activity_timestamp) if self.last_activity_timestamp else None
        last_activity_dt = timestamp_to_local_datetime(self.last_activity_timestamp)
    ...
    def last_activity_timestamp_display(self):
        ...
        if not value:
            return None
        dt = timestamp_to_local_datetime(self.last_activity_timestamp)
        return mark_safe(f'<span title="{dt}">{formatted_duration(int(now().timestamp()) - value)}</span>')
    ...
    def first_buy_timestamp_display(self):
        ...
        if not value:
            return None
        dt = timestamp_to_local_datetime(self.first_buy_timestamp)
        return mark_safe(f'<span title="{dt}">{formatted_duration(int(now().timestamp()) - value)}</span>')
    ...
    def first_sell_timestamp_display(self):
        ...
        if not value:
            return None
        dt = timestamp_to_local_datetime(self.first_sell_timestamp)
        return mark_safe(f'<span title="{dt}">{formatted_duration(int(now().timestamp()) - value)}</span>')
