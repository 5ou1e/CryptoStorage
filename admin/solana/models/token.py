...
class Token(models.Model):
    ...
    symbol = models.CharField(
        max_length=255, null=True, blank=True, verbose_name="Символ"
    )
    uri = models.URLField(
        max_length=200,
        null=True,
    ...
    async def bulk_import_tokens(cls, tokens):
        ...
        for token in tokens:
            _tokens_to_create[token["address"]] = cls(
                ...
                name=(
                    token["symbol"].replace("\x00", "") if token.get("symbol") else None
                ),
                decimals=token["decimals"] if token.get("decimals") else None,
                logo=(
                    token["logo"][:500].replace("\x00", "")
                    if token.get("logo")
                    else None
                ),
        ...
        if tokens_to_create:
            ...
            await cls.objects.abulk_create(
                tokens_to_create, batch_size=1000, ignore_conflicts=True
            )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["token", "minute"], name="unique_tokenprice_token_minute"
            )
        ]
