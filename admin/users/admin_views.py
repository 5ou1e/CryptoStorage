...
@login_required
def update_wallet_stats_task_status_view(request, wallet_id, task_id):
    task_result = TaskResult.objects.filter(task_id=task_id).first()

    if task_result.status == 'SUCCESS':
        try:
            result = json.loads(task_result.result)  # Если результат в формате JSON
...
def toggle_wallet_status(request, wallet_id):
    if request.method == 'POST':
        ...
        try:
            ...
            user_wallet, created = UserWallet.objects.get_or_create(user=request.user, wallet_id=wallet_id)
...
def update_remark(request, wallet_id):
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({'status': 'success', 'message': 'Описание успешно сохранено!'})
        except json.JSONDecodeError:
